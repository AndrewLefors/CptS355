...
def read_expr(src):
    ...
    # create a PSString object having this string value.
        ...
        while count > 0:
            ...
            if next_token == '(':
                ...
                # If the token is a number or a string, add it to the string with a space separator.
                if is_literal(next_token) or is_name(next_token):
                    str_list = str_list + str(next_token) + " "
                # If the token is a string delimiter or an integer, add it to the string without a space separator.
                else:
                    str_list = str_list + str(next_token)
        str_list = str_list[:-1]  # Remove the extra space at the end of the string
        psVar = PSString(str_list)
        return psVar
    ...
    # create a PSCodeArray object having this list value.
        ...
        arr_list = read_block_expr(src)
        codeVar = PSCodeArray(arr_list)
        return codeVar

    # If the token is a number, create a PSNumber object having `number` token.
    else:
